...
export default function Conference2026TestView() {
    ...
    const t = {
        ...
        grantNotice: isEn
            ...
            : "ღონისძიება ხორციელდება საქართველოს შოთა რუსთაველის ეროვნული სამეცნიერო ფონდის მხარდაჭერით [გრანტის ნომერი: ISE-26-286].",
        freeNotice: isEn
    ...
    return (
        <div className="bg-slate-50 min-h-screen pb-20">
            {/* Top Sandbox & Logo Evaluation Control Banner */}
            <div className="sticky top-0 z-50 bg-gradient-to-r from-[#2D1540] via-[#3d1958] to-[#2D1540] text-white border-b border-amber-400/30 py-2.5 px-4 sm:px-6 lg:px-8 shadow-lg backdrop-blur-md">
                <div className="max-w-[1600px] mx-auto flex flex-col md:flex-row items-center justify-between gap-2.5">
                    <div className="flex items-center gap-3">
                        <div className="w-8 h-8 rounded-xl bg-gradient-to-br from-amber-400 to-amber-500 text-slate-950 flex items-center justify-center font-black text-xs shadow-xs flex-shrink-0">
                            70
                        </div>
                        <div className="text-left">
                            <div className="flex items-center gap-2">
                                <span className="text-[10px] font-black uppercase px-2 py-0.5 rounded-full bg-emerald-500/20 text-emerald-300 border border-emerald-500/40">
                                    {isEn ? "SANDBOX ROUTE" : "სატესტო როუტი"}
                                </span>
                                <span className="text-xs font-black text-amber-300 tracking-tight">
                                    {isEn ? "70th Jubilee Logo Redesign (Ribbon-Free Options)" : "70 წლის საიუბილეო ლოგოს რედიზაინი (ლენტის გარეშე)"}
                                </span>
                            </div>
                            <p className="text-[11px] text-purple-200">
                                {isEn 
                                    ? "Organizing Committee Evaluation: Dr. Grigor Tatishvili & Dr. Nikoloz Nioradze"
                                    : "კომიტეტის განხილვა: დოქტ. გრიგორ ტატიშვილი & დოქტ. ნიკოლოზ ნიორაძე"}
                            </p>
                        </div>
                    </div>
                ...
                <div className="max-w-[1600px] mx-auto relative z-10">
                    <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 lg:gap-8 items-center">
                        ...
                        <div className="lg:col-span-7 space-y-3 text-center lg:text-left">
                            ...
                            {/* 70th Anniversary Clean Academic Dedication Badge (NO Heavy Ribbon) */}
                            <div className="inline-flex items-center gap-3 bg-purple-50/90 border border-purple-200/80 rounded-2xl py-2.5 px-4 shadow-2xs max-w-2xl text-left">
                                <Award className="w-5 h-5 text-[#60318e] flex-shrink-0" />
                                <span className="text-xs sm:text-sm font-black text-purple-950 leading-snug tracking-tight">
                                    {t.anniversary}
                                </span>
                            </div>
                        ...
                        {/* Right Column: 70th Anniversary Emblem Showcase (Options A, B, C) */}
                        <div className="lg:col-span-5 flex flex-col items-center justify-center relative">
                            {/* Active Option Badge & Contrast Toggles */}
                            <div className="mb-2 flex items-center justify-between w-full max-w-[420px] sm:max-w-[480px] lg:max-w-[520px] px-1">
                                <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-black bg-amber-100 text-amber-950 border border-amber-300 shadow-2xs">
                                    ...
                                    <span>{isEn ? LOGO_OPTIONS[selectedLogoOption].badgeEn : LOGO_OPTIONS[selectedLogoOption].badgeKa}</span>
                                </span>
                                
                                {/* Background Surface Switcher */}
                                <div className="flex items-center gap-1 bg-white/90 backdrop-blur-xs p-0.5 rounded-xl border border-purple-200 text-[11px] font-bold text-slate-600 shadow-2xs">
                                    <button
                                        type="button"
                                        onClick={() => setSurfaceBg('glass')}
                                        className={`px-2 py-0.5 rounded-lg transition-all ${surfaceBg === 'glass' ? 'bg-[#60318e] text-white shadow-2xs' : 'hover:text-[#60318e]'}`}
                                        title={isEn ? "Soft Glass Surface" : "შუშისებური ფონი"}
                                    >
                                        Glass
                                    </button>
                                    <button
                                        ...
                                        className={`px-2 py-0.5 rounded-lg transition-all ${surfaceBg === 'white' ? 'bg-[#60318e] text-white shadow-2xs' : 'hover:text-[#60318e]'}`}
                                        title={isEn ? "Pure White Surface" : "თეთრი ფონი"}
                                    >
                                        White
                                    </button>
                                    <button
                                        ...
                                        className={`px-2 py-0.5 rounded-lg transition-all ${surfaceBg === 'dark' ? 'bg-[#2D1540] text-amber-300 shadow-2xs' : 'hover:text-[#60318e]'}`}
                                        title={isEn ? "Dark Purple Surface" : "მუქი მეწამული ფონი"}
                                    >
                                        Dark
                                    </button>
                                </div>
                            ...
                            {/* Emblem Container Card with Dynamic Background */}
                            <div className={`relative p-6 sm:p-8 rounded-3xl transition-all duration-300 flex flex-col items-center justify-center group w-full max-w-[420px] sm:max-w-[480px] lg:max-w-[520px] ${
                                surfaceBg === 'dark'
                                    ? 'bg-[#2D1540] border-2 border-purple-500/50 shadow-2xl'
                                    : surfaceBg === 'white'
                                    ? 'bg-white border-2 border-slate-200 shadow-xl'
                                    : 'bg-gradient-to-br from-white/95 via-purple-50/70 to-indigo-50/90 border-2 border-purple-200/80 backdrop-blur-sm shadow-xl'
                            }`}>
                                <Image
                                    key={`${selectedLogoOption}-${isEn ? 'en' : 'ka'}`}
                                    src={LOGO_OPTIONS[selectedLogoOption].imgSrc}
                                    alt={isEn ? LOGO_OPTIONS[selectedLogoOption].titleEn : LOGO_OPTIONS[selectedLogoOption].titleKa}
                                    width={520}
                                    height={260}
                                    priority
                                    className="w-full h-auto object-contain drop-shadow-xl select-none transform group-hover:scale-105 transition-transform duration-500"
                                />

                                {/* Option Mini Caption & Download Link */}
                                <div className="mt-4 pt-3 border-t border-purple-100/60 w-full flex items-center justify-between text-xs">
                                    <div className="flex flex-col text-left">
                                        <span className="font-extrabold text-[#60318e]">
                                            {isEn ? LOGO_OPTIONS[selectedLogoOption].shortNameEn : LOGO_OPTIONS[selectedLogoOption].shortNameKa}
                                        </span>
                                        <span className="text-[11px] text-slate-500 line-clamp-1">
                                            {isEn ? LOGO_OPTIONS[selectedLogoOption].styleEn : LOGO_OPTIONS[selectedLogoOption].styleKa}
                                        </span>
                                    </div>
                                    <a
                                        href={LOGO_OPTIONS[selectedLogoOption].imgSrc}
                                        download={LOGO_OPTIONS[selectedLogoOption].downloadName}
                                        className="inline-flex items-center gap-1 text-[11px] font-bold text-[#60318e] hover:text-[#AD49E1] px-2.5 py-1 rounded-lg bg-purple-50 hover:bg-purple-100 transition-all flex-shrink-0"
                                        title={isEn ? "Download high-res PNG" : "ჩამოტვირთე მაღალი ხარისხის PNG"}
                                    >
                            ...
                            <div className="flex flex-col group">
                                ...
                                >
                                    ...
                                    <div className="flex-1 py-3 px-3 sm:px-5 flex flex-col justify-center min-w-0 select-none">
                                        ...
                                        <p className="text-[10px] sm:text-[11px] text-amber-300 font-semibold mt-1 leading-snug">
                                            {isEn
                                                ? "The event is supported by the Shota Rustaveli National Science Foundation of Georgia [Grant number ISE-26-286]."
                                                : "ღონისძიება ხორციელდება საქართველოს შოთა რუსთაველის ეროვნული სამეცნიერო ფონდის მხარდაჭერით [გრანტის ნომერი: ISE-26-286]."}
                                        </p>
            ...
            <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 mt-14 pt-8 border-t border-purple-100">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                    ...
                    >
                        ...
                        <span className="text-[10px] font-extrabold text-amber-800 uppercase mb-1">{isEn ? "SRNSFG • Grant ISE-26-286" : "რუსთაველის ფონდი • გრანტი ISE-26-286"}</span>
                        <p className="text-[11px] sm:text-xs font-bold text-slate-800 leading-snug group-hover:text-[#60318e] transition-colors">
                            {isEn ? "Shota Rustaveli National Science Foundation of Georgia" : "საქართველოს შოთა რუსთაველის ეროვნული სამეცნიერო ფონდი"}
                        </p>
                    </a>
                </div>
            </div>
                                        ...
                                        <div className="flex items-center justify-between">
                                            ...
                                            {selectedLogoOption === opt.id && (
                                                <span className="inline-flex items-center gap-1 text-[10px] font-black uppercase text-emerald-700 bg-emerald-100 px-2 py-0.5 rounded-full border border-emerald-300">
                                                    ...
                                                    {isEn ? "Active on Page" : "არჩეულია საიტზე"}
                                                </span>
                                            )}
                                        </div>
                                        ...
                                        <button
                                            ...
                                            className={`w-full py-2 px-3 rounded-xl text-xs font-black transition-all cursor-pointer flex items-center justify-center gap-1.5 ${
                                                selectedLogoOption === opt.id
                                                    ...
                                                    : 'bg-white hover:bg-purple-100 text-[#60318e] border border-purple-200'
                                            }`}
                                        >
